...
for i, row in df_total.iterrows():
    ...
    # 💾 salvamento frequente (MUITO IMPORTANTE)
    if (i + 1) % 10 == 0:
        pd.DataFrame(resultados).to_excel(arquivo_saida, index=False)
        print("💾 Salvamento checkpoint")

    fim_noticia = time.time()
    print(f"⏱️ {fim_noticia - inicio_noticia:.2f}s")

# =========================
# 💾 FINAL (primeiro salvamento)
# =========================
df_final = pd.DataFrame(resultados)

# 🔧 corrigir tipos ANTES de salvar
colunas_bool = [
...
# 🔧 MUITO IMPORTANTE (evita crash aqui também)
for col in colunas_bool:
    if col in df_final.columns:
        df_final[col] = df_final[col].astype("object")
...
for idx in erros.index:
    ...
    # 🔧 atualizar linha (agora não quebra mais)
